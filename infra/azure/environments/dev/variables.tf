...
variable "subscription_id" {
  description = "Azure Subscription ID donde se creara el ambiente dev."
  type        = string
}
...
variable "environment" {
  description = "Nombre del ambiente. Este root module debe quedarse en dev."
  type        = string
  default     = "dev"

  validation {
    condition     = var.environment == "dev"
    error_message = "Este root module es solo para dev. Crear staging/prod en sus carpetas."
  }
}
...
variable "static_web_app_sku_tier" {
  description = "Tier de Static Web App. Free para aprendizaje/dev inicial."
  type        = string
  default     = "Free"
}
...
variable "log_retention_days" {
  description = "Dias de retencion de Log Analytics. 30 mantiene costo bajo."
  type        = number
  default     = 30
}
...
variable "enable_static_web_app" {
  description = "Crea Azure Static Web Apps. En algunas suscripciones dev puede estar bloqueado por policy regional."
  type        = bool
  default     = false
}
